...
(* we should cache, for scraping one file *)
let loc_digest_cache = Hashtbl.create 17
  
let reset_loc_digest_cache () = Hashtbl.clear loc_digest_cache

let loc_digest loc = match loc.Location.loc_start.Lexing.pos_fname with
  | "" -> None
  | f ->
...
let def path loc =
  Def (Hashcons.out_ident path, Hashcons.location_t loc, loc_digest loc)

let cache = Hashtbl.create 107
  
let reset_path_cache () = Hashtbl.clear cache

let top_path = ref None

let iname i = Hashcons.string i.Ident.name

let func_args = Hashtbl.create 17
let reset_func_args () = Hashtbl.clear func_args

let iname_func_arg i = match !top_path with
  | None -> assert false
  | Some tp ->
...
module Typedtree = struct
  ...
  let rec pattern c pat = pattern_desc c pat.pat_desc (* XXX extra? *)
...
*)
  ...
  and pattern_desc c = function
    | Tpat_any -> []
    | Tpat_var (id, {loc}) -> [(Sig.KValue, iname id), expr & EValue (def (o_dot c id) loc)]
    | Tpat_alias (p,id,{loc}) -> ((KValue, iname id), expr & EValue (def (o_dot c id) loc)) :: pattern c p
    | Tpat_constant _ -> []
...
*)
  ...
  and class_field c cf = class_field_desc c cf.cf_loc cf.cf_desc
...
*)
  ...
  and class_field_desc c _loc = function
(*
...
*)
    ...
    | Tcf_method (n, _private_flag, _class_field_kind) ->
        [(KMethod, n.txt), expr & EMethod (def (Oide_dot (c, n.txt)) n.loc)]
          ...
          | Tstr_include id ->
              ...
              ELet (false, [(KModule, "*include*"), m],
                    fold_right (fun (k,n as id) e ->
                      ELet (false, [id, EAddAlias(Def(Oide_dot(c,snd id),si.str_loc,loc_digest si.str_loc), EDot(expr & EVar(KModule, "*include*"), k, Hashcons.string n))], e))
                      vs e)
...
*)
  ...
  and module_binding c mb =
    ...
    (KModule, iname mb.mb_id), EModule (def c' mb.mb_name.loc, module_expr c' mb.mb_expr)
    ...
    let rec f rev_vs = function
      ...
      | si::sis ->
          ...
          | Tsig_include id ->
              ...
              ELet (false, [(KModule, "*include*"), m],
                    fold_right (fun (k,n as id) e ->
                      ELet (false, [id, EAddAlias(Def (Oide_dot(c,snd id),si.sig_loc, loc_digest si.sig_loc), EDot (expr & EVar (KModule, "*include*"), k, Hashcons.string n))], e))
                      vs e)
          | Tsig_class cds ->
...
*)
  and module_declaration c md =
    ((KModule, iname md.md_id), EModule (def (o_dot c md.md_id) md.md_name.loc, module_type c md.md_type))
...
*)
  ...
  and module_type_declaration c mtd =
    ...
    (KModtype, iname mtd.mtd_id), EModtype (def c' mtd.mtd_name.loc, Option.fmap (module_type c') mtd.mtd_type)
...
*)
  ...
  and value_description c vd =
    match vd.val_val.Types0.val_kind with
    | Types0.Val_prim { Primitive.prim_name } when prim_name.[0] = '%' ->
        (KValue, iname vd.val_id), expr & EValue (Aliased (def (o_dot c vd.val_id) vd.val_name.loc, Prim prim_name))
    | _ -> 
        (KValue, iname vd.val_id), expr & EValue (def (o_dot c vd.val_id) vd.val_name.loc)
...
*)
  ...
  and type_declaration c td =
    ...
    (KType, iname td.typ_id), expr & EType (def (o_dot c td.typ_id) td.typ_name.loc, stk)
...
*)
  ...
  and label_declaration c ld =
    (KField, iname ld.ld_id), expr & EField (def (o_dot c ld.ld_id) ld.ld_name.loc)
...
*)
  ...
  and constructor_declaration c cd =
    (KConstructor, iname cd.cd_id), expr & EConstructor (def (o_dot c cd.cd_id) cd.cd_name.loc)
...
*)
  ...
  and extension_constructor c ec =
    (KTypext, iname ec.ext_id), expr & ETypext (def (o_dot c ec.ext_id) ec.ext_name.loc)
...
*)
  ...
  and class_type_field c ctf = class_type_field_desc c ctf.ctf_loc ctf.ctf_desc
...
*)
  and class_declaration c ci =
    ...
    [ (KClass, ci.ci_id_name.txt), expr & EClass (def c' ci.ci_id_name.loc, meths)
    ; (KClasstype, ci.ci_id_name.txt), expr & EClasstype (def c' ci.ci_id_name.loc, meths)
    ; (KType, ci.ci_id_name.txt), expr & EType (def c' ci.ci_id_name.loc, [])
    ; (KType, "#" ^ ci.ci_id_name.txt), expr & EType (def c' ci.ci_id_name.loc, [])
    ]
...
let signature p s =
  top_path := Some p;
  reset_path_cache ();
  reset_loc_digest_cache ();
  Reset.typing ();
  Typedtree.signature p s
  |- fun _ ->
    top_path := None;
    reset_path_cache ();
    reset_func_args ()
